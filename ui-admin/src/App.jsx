                ...
                if (session?.expires_at) {
                    ...
                    if (expiresAt - now < 5 * 60 * 1000) {
                        ...
                        try {
                            ...
                            console.warn('⚠️  Failed to refresh session:', refreshError)
                        }
                    }
                }
            }
    ...
    const fetchFlights = async (retryCount = 0) => {
        ...
        try {
            ...
            if (res.status === 401 && retryCount === 0) {
                console.log('⚠️  401 Unauthorized, refreshing session and retrying...')
                try {
                    const { data, error } = await supabase.auth.refreshSession(session)
                    if (!error && data?.session) {
...
function AddFlightPage({ session, setView }) {
    ...
    const handlePredict = async () => {
        ...
        // Safety timeout - eğer 20 saniye içinde tamamlanmazsa zorla kapat
        let safetyTimeout
        try {
    ...
    return (
        <div>
            ...
            <div className="card flight-entry-form">
                <div className="card-title">✈️ Flight Entry</div>
